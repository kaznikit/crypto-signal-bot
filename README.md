...
Есть **walk-forward probe** для ветки разворота на **4H**. На каждом закрытом баре:
1. Считаем число подтверждённых CHoCH-событий через `bot.market.pivots.extract_structure_breaks`. Новое событие = выросло число.
2. Проверяем ATR.
3. Пытаемся собрать PREPARE через `detect_reversal_prepare` (Pine-импульс LH→LL + 0.5-mid).
4. Прогоняем через все включённые `strategy_features` гейты.
...
Вывод — четыре числа с воронкой:

```
1) New CHoCH events (fresh structural breaks)
2) Passed ATR + fresh-CHoCH window
3) PREPARE candidates (impulse + OTE built)
4) PREPARE after strategy_features gates
```
...
2. Симулировать ENTRY/FSM пост-PREPARE на 1H/15M для win-rate и R-кривой.
...
Помимо `symbols.count` и `strategy_features`: импульс continuation = последний leg HL→HH (LONG) или LH→LL (SHORT) в направлении свежего BOS/CHoCH (`pivots.swing_size_by_tf[htf]`), reversal — последний leg в направлении, **обратном** свежему CHoCH. PREPARE строится только на баре первого касания 0.5 импульса (`first_touch_of_level_since`); раньше continuation мог ARMиться вне зоны через фазу `WAIT_OTE`, теперь такого нет. Окно CHoCH на 4H согласовано с `reversal.choch_lookback_bars`; LTF CHoCH для ENTRY — Pine же, через `detect_ltf_choch` (`entry.ltf_swing_length`, `paper_mode.liberal.ltf_swing_length_override`); параллельные сетапы дедуплицируются по `(symbol, type, htf, direction)`.
...
`(symbol, type, htf, direction)`: пока активен (`ARMED`) сетап того же направления
на том же HTF, новые PREPARE этого направления не создаются. Это вырезает спам
вида «PREPARE на каждом 4H закрытии». Дедуп применяется и в live-боте, и в `history_replay`.
...
1. **Свежий структурный пробой** на HTF (Pine BOS/CHoCH через
   ...
   - **Continuation** — BOS или CHoCH в сторону тренда в окне
     `continuation.structure_max_bars_ago` HTF-баров (по умолчанию `120`).
...
PREPARE-сигнал срабатывает на баре первого касания. Дальше — ручной мониторинг
LTF; пока пользователь не увидит паттерн входа, никаких автоматических действий
нет. Лейбл `P LONG/SHORT` ставится ровно на баре касания на уровне 0.5 импульса.
